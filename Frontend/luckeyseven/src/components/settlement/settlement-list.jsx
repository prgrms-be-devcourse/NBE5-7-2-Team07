...
export function SettlementList({ settlements: initialSettlements }) {
  ...
  const handleUpdate = (updatedSettlement) => {
    setSettlements((prevSettlements) =>
      prevSettlements.map((settlement) => (settlement.id === updatedSettlement.id ? updatedSettlement : settlement)),
    )
  }
  ...
  return (
    <div className="space-y-4">
      {settlements.map((settlement) => (
        <Link to={`/settlements/${settlement.id}`} key={settlement.id} className="block">
          <div className="card hover:shadow-md transition cursor-pointer">
            <div className="card-content p-6">
              <div className="flex justify-between items-start">
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    <h3 className="font-medium">정산 #{settlement.id.substring(0, 8)}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    <div>
                      ...
                      <p className="font-medium">{settlement.payer?.name || "알 수 없음"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted">정산자</p>
                      <p className="font-medium">{settlement.settler?.name || "알 수 없음"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted">지출 항목</p>
                      <p className="font-medium">{settlement.expense?.title || "알 수 없음"}</p>
                    </div>
                  </div>
                </div>

                <div className="flex flex-col items-end space-y-4">
                  <p className="text-xl font-bold">{formatCurrency(settlement.amount)}</p>

                  <div className="flex space-x-2">
                    <SettlementActions settlement={settlement} onUpdate={handleUpdate} inline={true} />
