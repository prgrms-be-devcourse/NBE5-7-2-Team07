...
import { createSettlement, updateSettlement } from "../../service/settlementService"

export function SettlementForm({ settlement, users, expenses, isEditing = false }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.payerId || !formData.settlerId || !formData.expenseId || !formData.amount) {
      addToast({
        title: "입력 오류",
    ...
    try {
      ...
      addToast({
        ...
        description: isEditing ? "정산 내역이 성공적으로 수정되었습니다." : "새로운 정산 내역이 생성되었습니다.",
      })
  ...
  // 선택된 지출 항목 정보
  const selectedExpense = formData.expenseId ? expenses.find((expense) => expense.id === formData.expenseId) : null
    ...
    <form onSubmit={handleSubmit}>
      <div className="card">
            ...
            <div className="space-y-4">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) => handleChange("settlerId", e.target.value)}
                  disabled={isLoading}
                >
            ...
            <div className="space-y-4">
              <div className="form-group">
                ...
                <div className="h-6 mt-1">
                  {selectedExpense ? (
                      <p className="text-sm text-muted">총 지출 금액: {formatCurrency(selectedExpense.amount)}</p>
                  ) : (
        ...
        <div className="card-footer">
          <button type="button" className="btn btn-outline" onClick={() => navigate(-1)} disabled={isLoading}>
            취소
          </button>
