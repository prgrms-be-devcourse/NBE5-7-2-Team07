import { settlements, users, expenses } from "./mockData"
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if (filters.payerId && filters.payerId !== "all") {
        filteredSettlements = filteredSettlements.filter((s) => s.payerId === filters.payerId)
      }

      if (filters.settlerId && filters.settlerId !== "all") {
        filteredSettlements = filteredSettlements.filter((s) => s.settlerId === filters.settlerId)
      }

      if (filters.expenseId && filters.expenseId !== "all") {
        filteredSettlements = filteredSettlements.filter((s) => s.expenseId === filters.expenseId)
      }

      if (filters.isSettled !== undefined && filters.isSettled !== null && filters.isSettled !== "all") {
        const isSettledBool = filters.isSettled === "true" || filters.isSettled === true
        filteredSettlements = filteredSettlements.filter((s) => s.isSettled === isSettledBool)
      }
...
// 정산 수정
export const updateSettlement = async (id, settlementData, settledOnly = false) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (settledOnly) {
        ...
        settlements[index] = {
          ...
          ...settlementData,
          updatedAt: new Date().toISOString(),
          payer: settlementData.payerId ? users.find((u) => u.id === settlementData.payerId) : settlements[index].payer,
