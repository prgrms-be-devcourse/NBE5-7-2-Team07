...
export default function ExpenseList() {
  ...
  const handleUpdateSuccess = (upd, bal) => {
    setExpenses(prev => prev.map(e => e.id === upd.id ? upd : e));
    setBalances(bal);
    setNotification({ message: '지출이 성공적으로 수정되었습니다.', type: 'update' });
    closeDetail();
  };
  const handleDeleteSuccess = (delId, bal) => {
  ...
  if (loading) return (
    <div className="expense-tracker">
      <Header />
      <div className="content"><div className="loading">데이터를 불러오고 있습니다...</div></div>
    </div>
  );
  if (error) return (
    <div className="expense-tracker">
      ...
      <div className="content">
        ...
        {(balances || notification.message) && (
          <div className="balance-banner">
            {balances && (
              <div>
                <span className="label">원화 잔고:</span><strong>₩{fmt(balances.balance)}</strong>&nbsp;&nbsp;
                <span className="label">외화 잔고:</span><strong>${fmt(balances.foreignBalance)}</strong>
              </div>
            )}
        ...
        <div className="actions">
          ...
          <div className="sort-control">
            <button className="sort-btn" onClick={() => setSortDirection(prev => prev === 'DESC' ? 'ASC' : 'DESC')}>
          ...
          : (
            <div className="expense-table">
              <table>
                <thead>
                  <tr>
                    <th>제목</th><th>가격 (KRW)</th><th>카테고리</th><th>날짜</th><th>결제자</th>
                  </tr>
                </thead>
                <tbody>
                  {expenses.map(exp => (
        ...
        {totalPages > 1 && (
          <div className="pagination">
            ...
            {Array.from({ length: totalPages }, (_, i) => {
              const p = i + 1, cur = page + 1;
              if (p === 1 || p === totalPages || (p >= cur - 1 && p <= cur + 1)) {
                return <button key={i} className={p === cur ? 'active' : ''} onClick={() => goToPage(p)}>{p}</button>;
              }
              if (p === cur - 2 && cur > 3) return <span key="e1" className="pagination-ellipsis">...</span>;
              if (p === cur + 2 && cur < totalPages - 2) return <span key="e2" className="pagination-ellipsis">...</span>;
              return null;
            })}
